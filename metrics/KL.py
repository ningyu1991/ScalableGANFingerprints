...
class KL(metric_base.MetricBase):
    ...
    def _evaluate(self, Gs, Gs_kwargs, num_gpus):
        ...
        density_fake = np.histogram(labels_all, bins=np.arange(classifier.output_shape[1]+1), density=True)[0]
        density_real = np.ones(classifier.output_shape[1], dtype=np.float32) / float(classifier.output_shape[1])
        KL = np.sum(np.where(density_fake != 0, density_fake * np.log(density_fake / density_real), 0))
        self._report_result(KL)
