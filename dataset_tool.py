...
def create_celebahq(tfrecord_dir, out_img_dir, celeba_dir, delta_dir, num_threads=1, num_tasks=100, shuffle=1):
    print('Loading CelebA from "%s"' % celeba_dir)
    ...
    with TFRecordExporter(tfrecord_dir, indices.size) as tfr:
        order = tfr.choose_shuffled_order() if shuffle else np.arange(tfr.choose_shuffled_order().size)
        with ThreadPool(num_threads) as pool:
            if not os.path.isdir(out_img_dir): os.mkdir(out_img_dir)
            count = 0
            for img in pool.process_items_concurrently(indices[order].tolist(), process_func=process_func, max_items_in_flight=num_tasks):
                img_save = img.transpose(1,2,0)
                skimage.io.imsave('%s/%06d.png' % (out_img_dir, count), img_save)
                print('Generating png to %s: %d...\r' % (out_img_dir, count))
                count += 1
                #tfr.add_image(img)
...
def create_from_images(tfrecord_dir, image_dir, shuffle=1, resolution=256, num_images=0, num_shifts=0):
    ...
    if shuffle:
        np.random.RandomState(123).shuffle(image_filenames)
...
def execute_cmdline(argv):
    ...
    p.add_argument(     'tfrecord_dir',     help='New dataset directory to be created')
    p.add_argument(     'out_img_dir',      help='New png image directory to be created')
    p.add_argument(     'celeba_dir',       help='Directory containing CelebA')
