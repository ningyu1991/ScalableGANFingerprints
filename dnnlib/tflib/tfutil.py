...
def normalize(v: TfExpressionEx) -> TfExpressionEx:
    """Normalize batch of vectors."""
    with tf.name_scope("Normalize"):
        return v / tf.sqrt(tf.reduce_sum(tf.square(v), axis=-1, keepdims=True))

def slerp(a: TfExpressionEx, b: TfExpressionEx, t: TfExpressionEx) -> TfExpressionEx:
    """Spherical interpolation of a batch of vectors."""
    with tf.name_scope("SphericalLerp"):
        a = normalize(a)
        b = normalize(b)
...
def sdist(a: TfExpressionEx, b: TfExpressionEx) -> TfExpressionEx:
    """Spherical distance of a batch of vectors."""
    with tf.name_scope("SphericalDist"):
        a = normalize(a)
        b = normalize(b)
