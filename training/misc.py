...
def resume_kimg_time(network_pkl):
    ...
    with open('%s/log.txt' % path, 'r') as f:
        lines = f.readlines()
    for line in lines:
        if 'tick' in line and 'kimg' in line and 'minibatch' in line and 'time' in line and 'sec/tick' in line and 'sec/kimg' in line and 'maintenance' in line and 'gpumem' in line and (str(int(file[-6:])) in line or str(int(file[-6:])-1) in line):
            kimg = str(int(file[-6:]))
            idx = line.find(kimg)
            if idx < 0:
                kimg = str(int(file[-6:])-1)
                idx = line.find(kimg)
