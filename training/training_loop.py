...
def training_schedule(
    ...
    E_lrate_base            = 0.002,    # Learning rate for the generator.
...
def training_loop(
    ...
    EG_opt_args             = {},       # Options for generator optimizer.
    D_opt_args              = {},       # Options for discriminator optimizer.
    EG_loss_args            = {},       # Options for generator loss.
    ...
    total_kimg              = 88000,    # Total length of the training, measured in thousands of real images.
    ...
    image_snapshot_ticks = 30 # How often to save image snapshots? None = only save 'reals.png' and 'fakes-init.png'.
    network_snapshot_ticks = 150 # How often to save network snapshots? None = only save 'networks-final.pkl'.
    ...
    grid_watermarks = np.random.randint(low=0, high=2, size=tuple([np.prod(grid_size)]+G.input_shapes[2][1:])).astype(np.float32)
    grid_latents_same = np.tile(grid_latents[0,:], [np.prod(grid_size),1])
    grid_watermarks_same = np.tile(grid_watermarks[0,:], [np.prod(grid_size),1])
    ...
    # Setup optimizers.
    EG_opt_args = dict(EG_opt_args)
    D_opt_args = dict(D_opt_args)
    for args, reg_interval in [(EG_opt_args, G_reg_interval), (D_opt_args, D_reg_interval)]:
    ...
    E_opt = tflib.Optimizer(name='TrainE', **EG_opt_args)
    G_opt = tflib.Optimizer(name='TrainG', **EG_opt_args)
    D_opt = tflib.Optimizer(name='TrainD', **D_opt_args)
    G_reg_opt = tflib.Optimizer(name='RegG', share=G_opt, **EG_opt_args)
        ...
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            with tf.control_dependencies(lod_assign_ops):
                with tf.name_scope('EG_loss'):
                    EG_loss, G_reg = dnnlib.util.call_func_by_name(E=E_gpu, G=G_gpu, D=D_gpu, training_set=training_set, minibatch_size=minibatch_gpu_in, reals=reals_read, labels=labels_read, **EG_loss_args)
                with tf.name_scope('D_loss'):
                    D_loss, D_reg = dnnlib.util.call_func_by_name(E=E_gpu, G=G_gpu, D=D_gpu, opt=D_opt, training_set=training_set, minibatch_size=minibatch_gpu_in, reals=reals_read, labels=labels_read, **D_loss_args)

            # Register gradients.
            if not lazy_regularization:
                if G_reg is not None: EG_loss += G_reg
                if D_reg is not None: D_loss += D_reg
            else:
                if G_reg is not None: G_reg_opt.register_gradients(tf.reduce_mean(G_reg * G_reg_interval), G_gpu.trainables)
                if D_reg is not None: D_reg_opt.register_gradients(tf.reduce_mean(D_reg * D_reg_interval), D_gpu.trainables)
            E_opt.register_gradients(tf.reduce_mean(EG_loss), E_gpu.trainables)
            G_opt.register_gradients(tf.reduce_mean(EG_loss), G_gpu.trainables)
    ...
    if save_weight_histograms:
        E.setup_weight_histograms(); G.setup_weight_histograms(); D.setup_weight_histograms()
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        if reset_opt_for_new_lod:
            if np.floor(sched.lod) != np.floor(prev_lod) or np.ceil(sched.lod) != np.ceil(prev_lod):
                E_opt.reset_optimizer_state(); G_opt.reset_optimizer_state(); D_opt.reset_optimizer_state()
        prev_lod = sched.lod
        ...
        for _repeat in range(minibatch_repeats):
            ...
            if len(rounds) == 1:
                tflib.run([E_train_op, G_train_op, data_fetch_op], feed_dict)
                if run_G_reg:
            ...
            # Slow path with gradient accumulation.
                ...
                for _round in rounds:
                    tflib.run(E_train_op, G_train_op, feed_dict)
                if run_G_reg:
                    for _round in rounds:
                        tflib.run(G_reg_op, feed_dict)
    ...
    # Save final snapshot.
    misc.save_pkl((E, G, D, Gs), dnnlib.make_run_dir_path('network-final.pkl'))
