...
#----------------------------------------------------------------------------
# Main generator network enabling watermark input.
# Composed of two sub-networks (mapping and synthesis) that are defined below.
# Used in configs B-F (Table 1).

def G_main_watermark(
    latents_in,                                         # First input: Latent vectors (Z) [minibatch, latent_size].
    labels_in,                                          # Second input: Conditioning labels [minibatch, label_size].
    watermark_in,                                       # Third input: User-specific watermark [minibatch, nf(1), 4, 4]
    watermark_size          = 32,                       # Watermark dimensionality.
    truncation_psi          = 0.5,                      # Style strength multiplier for the truncation trick. None = disable.
    truncation_cutoff       = None,                     # Number of layers for which to apply the truncation trick. None = disable.
    truncation_psi_val      = None,                     # Value for truncation_psi to use during validation.
    truncation_cutoff_val   = None,                     # Value for truncation_cutoff to use during validation.
    ...
    is_template_graph       = False,                    # True = template graph constructed by the Network class, False = actual evaluation.
    components              = dnnlib.EasyDict(),        # Container for sub-networks. Retained between calls.
    mapping_func            = 'G_mapping_watermark',              # Build func name for the mapping network.
    synthesis_func          = 'G_synthesis_stylegan2',  # Build func name for the synthesis network.
    ...
    **kwargs):                                          # Arguments for sub-networks (mapping and synthesis).
    ...
    # Setup components.
    if 'synthesis' not in components:
        components.synthesis = tflib.Network('G_synthesis', func_name=globals()[synthesis_func], res_modulated_min_log2=res_modulated_min_log2, res_modulated_max_log2=res_modulated_max_log2, **kwargs)
    num_layers = components.synthesis.input_shapes[1][1]
    dlatent_size = components.synthesis.input_shapes[1][2]
    if 'mapping' not in components:
        components.mapping = tflib.Network('G_mapping', func_name=globals()[mapping_func], watermark_size=watermark_size, dlatent_broadcast=num_layers, **kwargs)
    ...
    # Evaluate mapping network.
    dlatents = components.mapping.get_output_for(labels_in, watermark_in, is_training=is_training, **kwargs)
    dlatents = tf.cast(dlatents, tf.float32)
    ...
    #if style_mixing_prob is not None:
    #    with tf.variable_scope('StyleMix'):
    #        watermark2 = tf.random_normal(tf.shape(watermark_in))
    #        dlatents2 = components.mapping.get_output_for(labels_in, watermark2, is_training=is_training, **kwargs)
    #        dlatents2 = tf.cast(dlatents2, tf.float32)
    #        layer_idx = np.arange(num_layers)[np.newaxis, :, np.newaxis]
    #        cur_layers = num_layers - tf.cast(lod_in, tf.int32) * 2
    #        mixing_cutoff = tf.cond(
    ...
    if 'lod' in components.synthesis.vars:
        deps.append(tf.assign(components.synthesis.vars['lod'], lod_in))
    with tf.control_dependencies(deps):
        images_out = components.synthesis.get_output_for(latents_in, dlatents, is_training=is_training, force_clean_graph=is_template_graph, **kwargs)
...
def G_mapping_watermark(
    labels_in,                              # First input: Conditioning labels [minibatch, label_size].
    watermark_in,                           # Second input: Conditioning watermarks [minibatch, latent_size].
    watermark_size          = 32,           # Watermark dimensionality.
    label_size              = 0,            # Label dimensionality, 0 if no labels.
    dlatent_size            = 512,          # Disentangled latent (W) dimensionality.
    dlatent_broadcast       = None,         # Output disentangled latent (W) as [minibatch, dlatent_size] or [minibatch, dlatent_broadcast, dlatent_size].
    ...
    **_kwargs):                             # Ignore unrecognized keyword args.
    ...
    # Inputs.
    labels_in.set_shape([None, label_size])
    watermark_in.set_shape([None, watermark_size])
    labels_in = tf.cast(labels_in, dtype)
    watermark_in = tf.cast(watermark_in, dtype)
    x = watermark_in

    # Embed labels and concatenate them with latents.
    if label_size:
        with tf.variable_scope('LabelConcat'):
            w = tf.get_variable('weight', shape=[label_size, watermark_size], initializer=tf.initializers.random_normal())
            y = tf.matmul(labels_in, tf.cast(w, dtype))
            x = tf.concat([x, y], axis=1)
...
def G_synthesis_stylegan2(
    latents_in,                         # First input: Latent vectors (Z) [minibatch, latent_size].
    dlatents_in,                        # Second input: Disentangled latents (W) [minibatch, num_layers, dlatent_size].
    latent_size         = 512,          # Latent vector (Z) dimensionality.
    dlatent_size        = 512,          # Disentangled latent (W) dimensionality.
    num_channels        = 3,            # Number of output color channels.
    resolution          = 1024,         # Output resolution.
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    # Single convolution layer with all the bells and whistles.
    def layer(x, layer_idx, fmaps, kernel, up=False, modulated=True):
        if modulated:
            x = modulated_conv2d_layer(x, dlatents_in[:, layer_idx], fmaps=fmaps, kernel=kernel, up=up, resample_kernel=resample_kernel, fused_modconv=fused_modconv)
        else:
    ...
    def block(x, res): # res = 3..resolution_log2
        t = x
        with tf.variable_scope('Conv0_up'):
            if res >= res_modulated_min_log2 and res <= res_modulated_max_log2:
                x = layer(x, layer_idx=res*2-5, fmaps=nf(res-1), kernel=3, up=True, modulated=True)
            else:
                x = layer(x, layer_idx=res*2-5, fmaps=nf(res-1), kernel=3, up=True, modulated=False)
        with tf.variable_scope('Conv1'):
            if res >= res_modulated_min_log2 and res <= res_modulated_max_log2:
                x = layer(x, layer_idx=res*2-4, fmaps=nf(res-1), kernel=3, modulated=True)
            else:
                x = layer(x, layer_idx=res*2-4, fmaps=nf(res-1), kernel=3, modulated=False)
    ...
    with tf.variable_scope('4x4'):
        with tf.variable_scope('Latent'):
            x = apply_bias_act(dense_layer(latents_in, fmaps=nf(1)*4*4), act=act)
            x = tf.reshape(x, [-1, nf(1), 4, 4])
        with tf.variable_scope('Conv'):
            if 2 >= res_modulated_min_log2 and 2 <= res_modulated_max_log2:
                x = layer(x, layer_idx=0, fmaps=nf(1), kernel=3, modulated=True)
            else:
                x = layer(x, layer_idx=0, fmaps=nf(1), kernel=3, modulated=False)
...
#----------------------------------------------------------------------------
# StyleGAN2 binary encoder (Figure 7).
# Implements skip connections and residual nets (Figure 7), but no progressive growing.
# Used in configs E-F (Table 1).

def E_stylegan2_watermark(
    images_in,                          # First input: Images [minibatch, channel, height, width].
    labels_in,                          # Second input: Labels [minibatch, label_size].
    num_channels        = 3,            # Number of input color channels. Overridden based on dataset.
    resolution          = 1024,         # Input resolution. Overridden based on dataset.
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    # Output layer with label conditioning from "Which Training Methods for GANs do actually Converge?"
    with tf.variable_scope('Output'):
        x = apply_bias_act(dense_layer(x, fmaps=latent_size+watermark_size))
    latents_out = x[:,:latent_size]
    watermark_logits_out = x[:,latent_size:]

    # Output.
    assert latents_out.dtype == tf.as_dtype(dtype) and watermark_logits_out.dtype == tf.as_dtype(dtype)
    latents_out = tf.identity(latents_out, name='latents_out')
    watermark_logits_out = tf.identity(watermark_logits_out, name='watermark_logits_out')
